	...
	<body>
		...
		<div id="contents">
			...
			echo "<td>$srws[1]</td><td> </td>
					<td>$srws[5]</td><td> </td>
					<td><input type='text' name='s1im' size='3' value='$srws[6]'></td><td> </td>
					<td><center>$srws[7]</center></td><td> </td>
					<td><input type='submit' name='sub'  ></td></tr></table></form>";
					if(!empty($_REQUEST['sub'])){
							$s1ima=$_REQUEST['s1im'];
							echo "$s1ima";
							echo "$srws[5]";
							$ddd= " UPDATE student SET internal='{$s1ima}' WHERE student_roll='{$srws[5]}'";
							echo $ddd;
							...
							/*$dtd="INSERT INTO student VALUES('','{$srws[1]}','male','2015','ttkjm@gmail.com','1551199','{$s1ima}','')";
							$db->query($dtd) or die($db->error);
				if($db->affected_rows==1)
				{	echo "<p align='center'>";
